...
export const watchApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    getWatches: builder.query({
      query: ({ limit = 10, page = 1, sort = "name_asc" }) => {
        // Modified to accept dynamic `limit`, `page`, and `sort` values
        return `/api/watches?limit=${limit}&page=${page}&sort=${sort}`;
        // Default sort to "name_asc"
      },
    }),
    ...
    // Updated Search Endpoint - Corrected path to `/api/search-watches`
    getSearchWatches: builder.query({
      query: ({
        ...
        return `/search-watches?query=${encodeURIComponent(
          searchQuery
        )}&limit=${limit}&page=${page}&sort=${sort}`;
        // Here we're using the 'searchQuery', and supporting pagination and sorting
      },
    }),
...
export const {
  ...
  useGetWatchesQuery, // Hook for fetching watches (with pagination and sorting support)
